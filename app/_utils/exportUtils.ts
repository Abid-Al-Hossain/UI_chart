...
function gridBackground(config) {
  if (!config.showGrid) return "transparent";
  return "linear-gradient(to top, " + hexAlpha(config.gridColor, config.gridOpacity) + " 1px, transparent 1px) 0 0 / 100% 25%";
}
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
      {items.map((item, index) => (
        <span key={item} style={{ display: "inline-flex", alignItems: "center", gap: 8, background: config.legendBg, border: \`1px solid \${config.legendBorder}\`, borderRadius: 999, padding: "4px 10px", fontSize: 12, color: config.legendText }}>
...
function BarChart({ config, data }) {
  ...
  return (
    <div>
      {config.yAxisLabel ? <span style={{ display: "block", fontSize: 11, textTransform: "uppercase", letterSpacing: ".12em", color: config.labelColor, marginBottom: 8 }}>{config.yAxisLabel}</span> : null}
      <div style={{ minHeight: 192, display: "flex", alignItems: "end", gap: config.barGap, border: \`1px solid \${config.border}\`, borderRadius: 24, padding: 16, background: config.chartBg !== "transparent" ? config.chartBg : gridBackground(config) }}>
        {data.map((datum) => (
          <div key={\`\${datum.label}-\${datum.series}\`} style={{ flex: 1, display: "grid", alignContent: "end", gap: 8, textAlign: "center" }}>
            <span style={{ fontSize: config.labelSize, fontWeight: 700, color: config.labelColor }}>{formatChartValue(datum.value, config.valueFormat)}</span>
            <div style={{ height: Math.max(16, (datum.value / max) * 150), borderTopLeftRadius: config.barRadius, borderTopRightRadius: config.barRadius, background: datum.color, transition: config.transitionDuration > 0 ? "height " + config.animationDuration + "ms ease, width " + config.animationDuration + "ms ease" : "none" }} />
          </div>
        ))}
      </div>
      <AxisLabels config={config} labels={data.map((datum) => datum.shortLabel)} />
      {config.xAxisLabel ? <span style={{ display: "block", textAlign: "center", fontSize: 11, textTransform: "uppercase", letterSpacing: ".12em", color: config.labelColor, marginTop: 8 }}>{config.xAxisLabel}</span> : null}
    </div>
  );
}
...
function LineChart({ config, data }) {
  ...
  return (
    <svg viewBox={\`0 0 \${width} \${height + 34}\`} style={{ minHeight: 192, width: "100%", border: \`1px solid \${config.border}\`, borderRadius: 24, padding: 12, background: config.chartBg !== "transparent" ? config.chartBg : "transparent" }} aria-hidden="true">
      {config.showGrid ? [0, 0.25, 0.5, 0.75, 1].map((frac) => (
        <line key={frac} x1={0} x2={width} y1={height * frac + 10} y2={height * frac + 10} stroke={hexAlpha(config.gridColor, config.gridOpacity)} strokeWidth={1} strokeDasharray={dashArray(config.gridStyle)} />
      )) : null}
      <polyline fill="none" stroke={config.accent} strokeWidth={config.lineWidth} strokeLinecap="round" strokeLinejoin="round" points={points.map((point) => \`\${point.x},\${point.y}\`).join(" ")} />
      {points.map((point) => (
        <g key={\`\${point.label}-\${point.series}\`}>
          <circle cx={point.x} cy={point.y} r={config.pointSize} fill={point.color} stroke={config.pointColor} strokeWidth="3" />
          <text x={point.x} y={Math.max(12, point.y - 12)} textAnchor="middle" fontSize={config.labelSize + 8} fill={config.labelColor}>{formatChartValue(point.value, config.valueFormat)}</text>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "minmax(180px, 220px) 1fr", alignItems: "center", gap: 24 }}>
      <div style={{ position: "relative", width: 208, height: 208, borderRadius: 999, background: \`conic-gradient(\${segments})\` }}>
        <div style={{ position: "absolute", inset: config.donutThickness, display: "grid", placeItems: "center", textAlign: "center", borderRadius: 999, background: config.chartBg !== "transparent" ? config.chartBg : config.background }}>
          <strong style={{ fontSize: config.titleSize }}>{formatChartValue(total, config.valueFormat)}</strong>
          <span style={{ fontSize: 12, color: config.muted }}>{config.donutCenterText}</span>
        </div>
      </div>
...
function TooltipPreview({ config, datum }) {
  return <div style={{ width: "fit-content", border: \`1px solid \${config.tooltipBorder}\`, borderRadius: config.tooltipRadius, padding: "10px 14px", background: config.tooltipBg, color: config.tooltipText, fontSize: 14 }}><strong>{datum.label}</strong><p style={{ margin: "4px 0 0", color: config.tooltipText, opacity: 0.75 }}>{datum.series}: {formatChartValue(datum.value, config.valueFormat)}</p></div>;
}
...
function ChartBody({ config, data }) {
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      {config.showLegend && config.legendPosition === "top" ? <Legend config={config} /> : null}
      ...
      {config.showLegend && config.legendPosition === "bottom" ? <Legend config={config} /> : null}
...
cursor: state.disabled ? state.disabledCursor : undefined,
    ...
    >
      <div style={{ display: "flex", alignItems: "start", justifyContent: "space-between", gap: 16 }}>
        ...
        <span style={{ border: \`1px solid \${state.disabled && state.disabledUseCustomColors ? state.disabledBorder : state.border}\`, borderRadius: 999, padding: "4px 10px", color: state.accent, fontSize: 12, textTransform: "uppercase", letterSpacing: ".14em" }}>{state.chartType}</span>
      </div>
